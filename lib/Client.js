...
class Client {
  constructor(initialToken, synchronizationStrategy, initialMessageCount) {
    ...
    this._channelJoinedSubscription = NativeAppEventEmitter.addListener(
        'chatClient:channelJoined',
        (channel) => {
          if (this.onChannelJoined) this.onChannelJoined(new Channel(channel));
        },
    );
    ...
    this._channelMemberJoinedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onMemberJoined) this.onMemberJoined({ channelSid, member: new Member(member, channelSid) });
      },
    );
    ...
    this._channelMemberChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onMemberChanged) this.onMemberChanged({channelSid, member: new Member(member, channelSid) });
      },
    );
    ...
    this._channelMemberLeftSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onMemberLeft) this.onMemberLeft({ channelSid, member: new Member(member, channelSid) });
      },
    );
    ...
    this._ErrorReceivedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ error, user }) => {
        if (this.onError) this.onError({ error, user });
      },
    );
    ...
    this._typingChannelStartedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onTypingStarted) this.onTypingStarted({ channelSid, member: new Member(member, channelSid) });
      },
    );
    ...
    this._typingChannelEndedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onTypingEnded) this.onTypingEnded({ channelSid, member: new Member(member, channelSid) });
      },
    );
    ...
    this._toastRegistrationFailedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ error, user }) => {
        if (this.onToastFailed) this.onToastFailed({ error, user });
      },
    );

    this._userUpdatedSubscription = NativeAppEventEmitter.addListener(
      'chatClient:userUpdated',
      ({ updated, user }) => {
        this.user = new User(user);
        if (this.onUserUpdated) this.onUserUpdated({ updated, user: this.user });
      },
    );

    this._channelMemberUserUpdatedSubscription = NativeAppEventEmitter.addListener(
      'chatClient:channel:member:userUpdated',
      ({ channelSid, updated, userDescriptor }) => {
        if (this.onMemberUserUpdated) {
          this.onMemberUserUpdated({ channelSid, updated, userDescriptor: new UserDescriptor(userDescriptor) });
        }
      },
    );
  }
  ...
  getSubscribedChannels() {
    return TwilioChatChannels.getSubscribedChannels();
  }
  ...
  _removeListeners() {
    ...
    this._channelJoinedSubscription.remove();
    ...
    this._channelMemberUserUpdatedSubscription.remove();
  }
}
