...
class Channel {
  constructor(props) {
    ...
    if (props.messagesCount) this.messagesCount = props.messagesCount;
    ...
    this._channelMemberJoinedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onMemberJoined) this.onMemberJoined(new Member(member, channelSid));
      },
    );
    ...
    this._channelMemberChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onMemberChanged) this.onMemberChanged(new Member(member, channelSid));
      },
    );
    ...
    this._channelMemberLeftSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onMemberLeft) this.onMemberLeft(new Member(member, channelSid));
      },
    );
    ...
    this._typingChannelStartedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onTypingStarted) this.onTypingStarted(new Member(member, channelSid));
      },
    );
    ...
    this._typingChannelEndedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onTypingEnded) this.onTypingEnded(new Member(member, channelSid));
      },
    );
    ...
    this._memberUserUpdatedSubscribtion = NativeAppEventEmitter.addListener(
      'chatClient:channel:member:userUpdated',
      ({ channelSid, updated, userDescriptor }) => {
          if (channelSid === this.sid && this.onMemberUserInfoUpdated) {
            this.onMemberUserInfoUpdated({ updated, userDescriptor: new UserDescriptor(userDescriptor) });
          }
      },
    );
  }
  ...
  getMember(identity) {
    ...
    .then(member => new Member(member, this.sid));
  }

  //very messed up method, need to have a look after twilio dev unifies the APIs
  async getMembers() {
    ...
    if(Platform.OS === 'ios') {
      ...
      .then(({ sid, type, paginator }) => new Paginator(this.sid, type, paginator));
    }
    const members = await TwilioChatMembers.getMembers(this.sid);
    var paginatorObj = {
      hasNextPage: false,
      items: members.items
    };
    // manual creation of the Paginator
    const paginator = new Paginator(this.sid, members.type, paginatorObj);
  ...
  close() {
    ...
    this._memberUserUpdatedSubscribtion.remove();
  }
}
